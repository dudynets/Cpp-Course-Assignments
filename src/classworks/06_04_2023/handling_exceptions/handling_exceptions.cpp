...
int main() {
  ...
  try {
    ...
    ofstream ofs("../src/classworks/06_04_2023/handling_exceptions/output.txt");
    ...
    if (name.empty() || name.size() > 20) {
      throw length_error("Invalid name");
    }
    ...
    if (choice < 1 || choice > languages.size() + 1) {
      throw invalid_argument("Invalid language choice");
    }
    ...
    cerr << "length_error: " << e.what() << "\n";
  }catch (const invalid_argument& e) {
    cerr << "invalid_argument: " << e.what() << "\n";
